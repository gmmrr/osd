...
class OnlineFeats(Dataset):
    ...
    def __init__(self, ami_audio_root, label_root, configs, segment=300, probs=None, synth=None):
        ...
        split_name = Path(label_root).name
        print(
            f"OnlineFeats[{split_name}]: using {len(devices)} audio files "
            f"from {len(devices_hash)} sessions; skipped {skipped_audio_files} "
            f"audio files from {len(skipped_sessions)} sessions without labels."
        )
    ...
    def normalize(signal, target_dB):
        """Peak-normalize audio to ``target_dB`` dBFS without invoking SoX.

        This matches the ``sox norm`` operation previously used through
        pysndfx, while avoiding its binary ``numpy.fromstring`` call that is
        incompatible with NumPy 2.x. Silent inputs remain silent.
        """
